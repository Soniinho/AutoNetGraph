...
def cloneConfigureMachines(selected_vm, scene):
    ...
    for item in scene.items():
        if isinstance(item, NetworkShape):  # Verifica se o item é uma forma com parâmetros de rede               
            clone_name = cloneMachine(vbox, source_vm, selected_vm, machine_name_comp)
            machine_name_comp += 1

            session = virtualbox.Session() # Abre a Sessão            
            machine = vbox.find_machine(clone_name) # Encontra a máquina virtual pelo nome
            ...
            # Realiza Login
            login(keyboard, username, password)
...
def login(keyboard, username, password):
